...
fputs($fp, "					×\n ");
fputs($fp, "				</button>\n -->");
fputs($fp, "				<h4 class='modal-title' id='myModalLabel'> Edit   </h4>\n ");
fputs($fp, "			</div>\n ");
...
fputs($fp, "		var html = \"<div class='row'><div class='col-md-12 ui_wrapper'><div id='row_\" + TABLE_DATA[i]['id'] + \"' >\";\n ");
fputs($fp, "		html += \"<div class='row'><h2>\" + table_header + \"</h2></div>\";\n ");
 	...
 	$max = count($arrayOfProperties);
		for ($x = 0; $x < $max; $x++) {
			...
			if($a_relation[1] == "id"){
				fputs($fp,"\n\t html += \"<div class='row'><div class='col-md-6 text-right' >".$arrayOfProperties[$x].":</div><div class='col-md-6 text-left'><input type='text' id='".$arrayOfProperties[$x]."'  name='".$arrayOfProperties[$x]."' class='info' value='\"+TABLE_DATA[i]['".$arrayOfProperties[$x]."']+\"' /> </div></div>\" ");
...
fputs($fp, "	   	html += \"<div class='col-md-3' ><a onclick='save_edit( \"  +TABLE_DATA[i]['id']+\");' class='panel_anchor'>save</a></div>\";\n ");
fputs($fp, "	   	html += \"<div class='col-md-3' ><a onclick='cancel_edit( \"+TABLE_DATA[i]['id']+\");' class='panel_anchor'>cancel</a></div>\";\n ");
fputs($fp, "	    html += \"<div class='col-md-3' ><a onclick='delete_user( \"+TABLE_DATA[i]['id']+\");' class='panel_anchor'>delete</a></div>\";\n ");
fputs($fp, "		html +=\"</div>\";\n ");

fputs($fp, "		html +=\"</div></div></div>\";\n ");
...
fputs($fp, "		var html = \"<div class='row'><div class='col-md-12 ui_wrapper'><div id='row_create' >\";\n ");
fputs($fp, "		html += \"<div class='row'><h2>\" + table_header + \"</h2></div>\"; \n");
fputs($fp, "		html += \"<div class='row'><h3>create:</h3></div>\";\n ");
fputs($fp, "		 ");
$max = count($arrayOfProperties);
		for ($x = 0; $x < $max; $x++) {
			fputs($fp,"\n\t html += \"<div class='row'><div class='col-md-6 text-right' >".$arrayOfProperties[$x].":</div><div class='col-md-6 text-left'><input type='text' id='".$arrayOfProperties[$x]."'  name='".$arrayOfProperties[$x]."' class='info' value='' /></div></div>\" \n");
 ...
 */
 fputs($fp, "		html +=\"<div class='row'>\"; \n");

fputs($fp, "\n		html += \" <input type='hidden' id='app_id'  name='app_id' class='info' value=''    /> \"; \n");
fputs($fp, "	   	html += \"<div class='col-md-3' ><a onclick='save_create();' class='panel_anchor'>save</a></div>\";\n ");
fputs($fp, "	   	html += \"<div class='col-md-3' ><a onclick='cancel_create();' class='panel_anchor'>cancel</a></div>\";\n ");
